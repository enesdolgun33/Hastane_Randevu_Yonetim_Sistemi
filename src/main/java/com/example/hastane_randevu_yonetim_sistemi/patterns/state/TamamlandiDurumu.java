...
public class TamamlandiDurumu implements IRandevuDurum {
    @Override
    public void durumuGuncelle(int randevuId) {
        String sql = "UPDATE randevular SET durum = 'Tamamlandı' WHERE id = ?";
        try {
            Connection conn = VeritabaniBaglantisi.getInstance().getConnection();
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, randevuId);
            ps.executeUpdate();
            System.out.println("State Deseni: Randevu #" + randevuId + " 'Tamamlandı' durumuna geçti.");
